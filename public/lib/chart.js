...
function Chart(props) {
  ...
  function _callAPI() {
    _callAPI = _asyncToGenerator(function* () {
      getTwitterData(); // const stock_data = await fetch("http://localhost:9000/stockAPI/" + timescale + "/" + stockIds).then(res => res.json())
  ...
  function drawLineGraph(stock_data, twitter_data) {
    ...
    if (!props.updateScale && !props.updateStocks) {
      ...
      svg.append("path").data([stock_data]).attr("d", currentline).attr("class", "chartLine"); // console.log(twitter_data);
