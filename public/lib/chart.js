...
function Chart(props) {
    ...
    function drawChart() {
        ...
        function drawLineGraph(stock_data, twit_data) {
            ...
            const minDate = d3.min(stock_data, function (d) {
                return utcToDate(d.date);
            });
            const maxDate = d3.max(stock_data, function (d) {
                return utcToDate(d.date);
            });
            const minPrice = d3.min(stock_data, function (d) {
                return parseFloat(d.close);
            });
            const maxPrice = d3.max(stock_data, function (d) {
                return parseFloat(d.close);
            });
            ...
            if (!props.updateScale) {
                ...
                .attr("transform", `translate(0, ${yTranslation})`).attr("class", "xAxis");

                svg.append("g").call(d3.axisLeft(priceScale)).attr("transform", `translate(${xTranslation}, 0)`).attr("class", "yAxis");
                ...
                .attr("transform", `translate(${yAxisX} ${yAxisY}) rotate(-90)`).text("Price (USD)").attr("class", "yAxisLabel");
            ...
            if (props.updateScale) {
                ...
                // Update lines
                svg.selectAll(".chartLine").duration(1000).attr("d", currentline);
                ...
                svg.selectAll("circle").duration(1000).attr("r", dotSize).attr("cx", function (d) {
                    ...
                    return priceScale(parseFloat(d.close));
                }).attr("stroke", "#FF0000").attr("fill", "#FF0000");

                // Update axes and labels
                svg.selectAll(".xAxisLabel").attr("x", xAxisX).attr("y", xAxisY);

                svg.selectAll(".yAxisLabel").attr("transform", `translate(${yAxisX} ${yAxisY}) rotate(-90)`);
                ...
                svg.selectAll(".yAxis").duration(1000).call(d3.axisLeft(priceScale)).attr("transform", `translate(${xTranslation}, 0)`);
